#ifndef BLOCK_3_COMMUNICATION_H
#define BLOCK_3_COMMUNICATION_H
...
message *receive_message(int socketfd, bool header) {
...
message *copy_msg(message *source){
    message *copy = message_template();
    copy->head->operation = source->head->operation;
    copy->head->key_length = source->head->key_length;
    copy->head->value_length = source->head->value_length;

    copy->key = malloc(sizeof(char) * copy->head->key_length);
    copy->value = malloc(sizeof(char) * copy->head->value_length);
    memcpy(copy->key, source->key, source->head->key_length * sizeof(char));
    memcpy(copy->value, source->value, source->head->value_length * sizeof(char));
    return copy;
}
